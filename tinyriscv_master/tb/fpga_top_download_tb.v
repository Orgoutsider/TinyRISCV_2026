...
// FPGA-top UART-download based PWM testbench.
// Flow:
...
module fpga_top_download_tb;
    ...
    // fpga_top only exports PWM_o[2:0]. PWM channel 3 is observed through
    // the internal SoC PWM bus so that the PWM check is identical to
    // tinyriscv_soc_pwm_tb.v.
    // assign pwm_o = fpga_top_0.pwm_o;
    ...
    task run_download;
        ...
        begin
            ...
            ready_wait = 0;
            // while (!((fpga_top_0.u_tinyriscv_soc_top.u_uart_debug.state == 5'd6 ||
            //           fpga_top_0.u_tinyriscv_soc_top.u_uart_debug.state == 5'd7) &&
            //          fpga_top_0.u_tinyriscv_soc_top.u_uart_debug.rx_index == 6'd0) &&
    ...
    //             if (fpga_top_0.u_fpga_mem_bridge.rom[i] !== fw_words[i]) begin
    //                 $display("ROM MISMATCH at word[%0d]: rom=0x%08x expected=0x%08x", i, fpga_top_0.u_fpga_mem_bridge.rom[i], fw_words[i]);
    ...
    initial begin
        ...
        // Bypass 20ms key debounce delay in simulation. Keep the effective
        // signals seen by tinyriscv_soc_top identical to the SoC-level UART
        // download PWM testbench.
        force fpga_top_0.rst_key_up = `RstEnable;      // SoC reset asserted, low active
        force fpga_top_0.debug_key_up = 1'b1;          // debug disabled, because fpga_top uses ~debug_key_up
        ...
        #200;
        rst = `RstDisable;
        force fpga_top_0.rst_key_up = `RstDisable;
        repeat (200) @(posedge clk);

        // Enable uart_debug inside fpga_top: tinyriscv_soc_top sees
        // uart_debug_pin = ~debug_key_up.
        uart_debug_pin = 1'b0;
        force fpga_top_0.debug_key_up = 1'b0;
        repeat (200) @(posedge clk);

        run_download();

        // Disable uart_debug before resetting/running the downloaded PWM program.
        uart_debug_pin = 1'b1;
        force fpga_top_0.debug_key_up = 1'b1;
        // check_rom_content();

        repeat (20) @(posedge clk);
        rst = `RstEnable;
        force fpga_top_0.rst_key_up = `RstEnable;
        repeat (20) @(posedge clk);
        rst = `RstDisable;
        force fpga_top_0.rst_key_up = `RstDisable;
        pwm_test_start = 1'b1;
    ...
    initial begin
        ...
        // if (fpga_top_0.u_tinyriscv_soc_top.u_pwm.enable !== 4'hF) begin
        //     pwm_pass = 0;
        //     $display("PWM enable mismatch: expected 0xF, actual 0x%0h", fpga_top_0.u_tinyriscv_soc_top.u_pwm.enable);
        // end
    ...
    // initial begin
    //     $dumpfile("fpga_top_download_tb.vcd");
    //     $dumpvars(0, fpga_top_download_tb);
    // end
